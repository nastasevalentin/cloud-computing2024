...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && path === '/books') {
    ...
    if (req.method === 'GET' && reqUrl.pathname.startsWith('/book/')) {
      ...
      if (req.method === 'POST' && path.startsWith('/book/')) {
        ...
        req.on('end', () => {
          try {
            ...
            if (!('title' in bookData && 'author' in bookData && 'published_date' in bookData && 'pages' in bookData)) {
              res.statusCode = 400;
              res.setHeader('Content-Type', 'application/json');
            ...
            const values = [bookData.title, bookData.author, bookData.published_date, bookData.pages];
            client.query(sql, values, (err, result) => {
              if (err) {
                console.error(err);
                res.statusCode = 500;
      ...
      else if (req.method === 'PUT' && path.startsWith('/movie/')) {
        let id = path.split('/')[2];
        console.log(id);
        let body = '';
